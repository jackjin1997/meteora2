...
export class TokenPrecisionConverter {
   ...
   */
  private async fetchDecimalsFromChain(
    tokenMint: PublicKey,
  ): Promise<number | null> {
    try {
  ...
  async convertPositionDataWithChainDecimals(
    ...
    const [tokenXDecimals, tokenYDecimals] = await this.batchGetTokenDecimals([
      tokenXMint,
      tokenYMint,
    ]);
    ...
    console.log(
      `   X代币精度: ${tokenXDecimals}, Y代币精度: ${tokenYDecimals}`,
    );

    return {
      tokenX: TokenPrecisionConverter.createTokenAmount(
        positionData.totalXAmount,
        tokenXDecimals,
      ),
      tokenY: TokenPrecisionConverter.createTokenAmount(
  ...
  static getDefaultDecimals(tokenSymbol?: string): number {
    ...
    return (
      this.DEFAULT_DECIMALS[symbol as keyof typeof this.DEFAULT_DECIMALS] ||
      this.DEFAULT_DECIMALS.DEFAULT
    );
  }
